...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_restartround", mpRestartRound, ADMFLAG_CHANGEMAP);
...
public OnStartTouch(const String:output[], ent, client, Float:delay)
{
	...
	if (GetConVarInt(ShowNoBlock)) PrintToChat(client, "Коснулись зоны!");
}

public OnEndTouch(const String:output[], ent, client, Float:delay)
{
	if (IsClientInGame(client)) SetEntProp(client, Prop_Data, "m_CollisionGroup", 6);
	if (GetConVarInt(ShowNoBlock)) PrintToChat(client, "Вышли из зоны!");
}

public OnPluginEnd() SetCommandFlags("kill", flagi);
...
{	
	if(!isMapRestartPending)
	{
		...
		{
			if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 2) PrintHintText(i, "%t", "restartmap", 5);
		}
		isMapRestartPending = true;	
		CreateTimer(5.0, timerRestartMap, _, TIMER_FLAG_NO_MAPCHANGE);
	}
...
public Action:mpRestartRound(client, args)
{
	new max = GetMaxClients();
	for(new i=1; i<=max; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 2)	PrintHintText(i, "%t", "restartround");
	}
