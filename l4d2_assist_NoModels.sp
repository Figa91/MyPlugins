...
public Plugin:myinfo = 
{
	...
	description = "Show assists made by survivors in killing Tank",
	...
	if (GetConVarInt(cvarEnable))
	{
		...
		if ((victim != 0) && (attacker != 0))
		{
			if(GetClientTeam(attacker) != 3 && GetClientTeam(victim) == 3)
			{
				...
				if (DamageHealth < 1024 && victim != attacker && GetClientTeam(victim) != GetClientTeam(attacker))
...
public Action:Event_Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarInt(cvarEnable))
	{
		new victim = GetClientOfUserId(GetEventInt(event, "userid"));
		new attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
		if ((victim != 0) && (attacker != 0) && GetClientTeam(victim) == 3 && GetClientTeam(attacker) != 3)
			{
				new class = GetEntProp(victim, Prop_Send, "m_zombieClass");
				if (class != ZOMBIECLASS_TANK)
					return Plugin_Handled;
		...
		if ((victim != 0) && (attacker != 0) && GetClientTeam(victim) == 3 && GetClientTeam(attacker) != 3)
			{
				...
				for (new i = 0; i <= MAXPLAYERS; i++)
				{
					...
					{
						if (i != attacker && IsClientInGame(i) && !IsFakeClient(i))
						{
							AssistFlag = true;
							decl String:tName[12];
				...
				PrintToChatAll("\x04Taнкa\x01 зaмaчuлu: \x05%s.", Message);
			}
		for (new i = 0; i <= MAXPLAYERS; i++)
			Damage[i][victim] = 0;
	}
...
public Action:Event_Witch_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarInt(cvarEnable))
	{
		...
		if (GetConVarInt(cvarWitch) && (Crownd==1))
			PrintToChatAll("\x05Зелёный эльф: \x04%N\x01 yгaндoшuл(a) \x04Вeдьмy.", attacker);
	}
	return Plugin_Continue;
}
